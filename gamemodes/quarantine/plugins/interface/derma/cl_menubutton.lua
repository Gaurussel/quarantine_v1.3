...
function PANEL:Init()
	self:SetFont("qCaptureMenuButtonLabelFont")
	self:SetTextColor(color_white)
	self:SetPaintBackground(false)
	...
	self.backgroundAlpha = 128
	self.currentBackgroundAlpha = 0
	self.backgroundChangeColor = ix.config.Get("color")
	self.backgroundChangeColor.a = 128
end
...
function PANEL:OnCursorEntered()
	...
	self:CreateAnimation(0.15, {
		target = {
			...
			backgroundColor = self.backgroundChangeColor
		}
	})
...
function PANEL:OnMouseReleased(key)
	...
	self:SetMaterialColor(color_black)
end

vgui.Register("qMenuButton", PANEL, "DButton")
...
function PANEL:Init()
	...
	self:SetFont("qCaptureMenuButtonLabelFont")
	self:SetContentAlignment(5)
	self.Matcolor = Color(0, 0, 0, 0)
end

function PANEL:Paint(width, height)
	if self.selected or self.cursorEntered then
		surface.SetDrawColor(Color(200, 200, 200, 30))
	...
	surface.DrawRect(0, height - 2, width, 16)
end
...
vgui.Register("qMenuSelectionButton", PANEL, "ixMenuSelectionButton")
...
function PANEL:AddButton(name, buttonList)
	...
	local button = self:Add("qMenuSelectionButton")
	button:SetMaterial(buttonMaterial)
	button:SetMaterialColor(Color(0, 0, 0))
	button.sectionParent = self.parent
	button:SetTextInset(buttonPadding * 2, 0)
	button:SetPadding(nil, 8, nil, 8)
	button:SetFont("ixCaptureCharSelFont")
...
vgui.Register("qMenuSelectionList", PANEL, "ixMenuSelectionList")

DEFINE_BASECLASS("qMenuButton")
PANEL = {}

function PANEL:Init()
	self:SetFont("ixCaptureMenuButtonLabelFont")
	self:SetTextColor(color_white)
	self:SetPaintBackground(false)
	self:SetContentAlignment(5)
	self:SetTextInset(buttonPadding, 0)
...
vgui.Register("qHelpMenuButton", PANEL, "qMenuButton")
...
vgui.Register("qMenuButton2", PANEL, "qMenuButton")
...
function PANEL:PaintBackground(width, height)
	if self.selected or self.cursorEntered then
		...
		surface.SetDrawColor(200, 200, 200, 30)
		surface.DrawTexturedRect(5, 0, width, height)
...
vgui.Register("qCraftingMenuButton", PANEL, "qMenuButton")
...
function PANEL:OnMousePressed(code)
	...
	self:SetMaterialColor(ix.config.Get("color"))
...
vgui.Register("qMainMenuButton", PANEL, "DButton")
