...
local function SetCharacter(self, character)
	...
	if (character) then
		...
		local bodygroups = character:GetPersonnalisation()

		if (istable(bodygroups)) then
			local faction = ix.faction.indices[character:GetFaction()]

			self:SetSkin(bodygroups["head"] or 0)
            self:SetBodygroup(1, faction.bodygroups[0][bodygroups["torso"]] or 0)
            self:SetBodygroup(2, faction.bodygroups[1][bodygroups["legs"]] or 0)
		else
...
function PANEL:Init()
	...
	continueButton:SetFont("ixMainMenuFont")
	...
	deleteButton:SetFont("ixMainMenuFont")
	...
	deleteConfirm.DoClick = function()
		...
		local canCharacterDelete, msg = hook.Run("CanCharacterDelete", LocalPlayer(), self.character)
		if canCharacterDelete == false then
			if msg then
				parent:ShowNotice(1, msg)
				self:Populate(id)
				self:SetActiveSubpanel("main")
			end
			return
	...
	deleteTitle:SetFont("ixCaptureTitleFont")
	...
	deleteText:SetFont("ixMainMenuFont")
