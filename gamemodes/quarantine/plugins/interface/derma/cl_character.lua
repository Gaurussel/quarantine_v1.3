...
local buttonMaterial = ix.util.GetMaterial("materials/menu/stroke-02.png")
...
function PANEL:Init()
	...
	self:SetBGColor(Color(45, 45, 45, 255))
	...
	local logo = ix.util.GetMaterial("materials/logo3.png")

	local logoImage = self:Add("DImage")
	logoImage:SetMaterial(logo)
	logoImage:SetSize(162, 162)
	logoImage:CenterVertical(0.3)
	logoImage:CenterHorizontal(0.5)
	logoImage:SetPaintedManually(true)

	local subtitleLabel = self:Add("DLabel")
	subtitleLabel:SetTextColor(color_white)
	subtitleLabel:SetFont("ixCaptureMenuButtonHugeFont")
	subtitleLabel:SetText("Зона карантина")
	subtitleLabel:SizeToContents()
	subtitleLabel:CenterVertical(0.4)
	subtitleLabel:CenterHorizontal(0.5)

	-- button list
	self.mainDPanel = self:Add("DPanel")
	self.mainDPanel:SetSize(ScrW() / 4, ScrH() / 3.5)
	self.mainDPanel:SizeToContents()
	--self.mainDPanel:SetPos(ScrW() * 0.3, ScrH() * 0.6)
	self.mainDPanel:CenterVertical(0.55)
	self.mainDPanel:CenterHorizontal(0.5)
	self.mainDPanel.Paint = function()
	end

	self.mainButtonList = self.mainDPanel:Add("ixCharMenuButtonList")
	self.mainButtonList:Dock(FILL)

	-- create character button
	local createButton = self.mainButtonList:Add("ixMenuButton")
	-- createButton:SetMaterialColor(Color(0, 0, 0))
	-- createButton:SetMaterial(buttonMaterial)
	createButton:SetText("create")
	createButton:SetFont("ixMainMenuTwoFont")
	...
	-- self.loadButton:SetMaterialColor(Color(0, 0, 0))
	-- self.loadButton:SetMaterial(buttonMaterial)
	self.loadButton:SetText("load")
	self.loadButton:SetFont("ixMainMenuTwoFont")
	...
	if (extraURL != "" and extraText != "") then
		...
		-- extraButton:SetMaterialColor(Color(0, 0, 0))
		-- extraButton:SetMaterial(buttonMaterial)
		extraButton:SetText(extraText, true)
		extraButton:SetFont("ixMainMenuTwoFont")
	...
	-- self.returnButton:SetMaterialColor(Color(0, 0, 0))
	-- self.returnButton:SetMaterial(buttonMaterial)
	self.returnButton:SetContentAlignment(5)
	self.returnButton:SetFont("ixMainMenuTwoFont")
...
function PANEL:Init()
	...
	self.background:OpenURL("https://screenload.myarena.site/assets/img/main_menuthree.png")]]
...
function PANEL:Close(bFromMenu)
	...
	local fadeOutTime = animationTime * 4
