...
local colorScheme = ix.config.Get("color")
local buttonMaterial = Material("materials/menu/stroke-01.png")
...
function PANEL:Init()
	...
	self.background:OpenURL("https://screenload.myarena.site/assets/img/photo13.png")]]
	self:SetBGColor(25, 25, 25, 255)
	...
	self.factionPanel.OnSetActive = function()
		...
		if (#self.factionButtons == 1) then
			self:SetActiveSubpanel("history", 0)
		end
	end
	...
	-- proceed:SetMaterialColor(Color(0, 0, 0))
	-- proceed:SetMaterial(buttonMaterial)
	proceed:SetFont("qCaptureMenuButtonLabelFont")
	...
	proceed.DoClick = function()
		...
		self:Populate()
		self:SetActiveSubpanel("history")
	end
	...
	-- factionBack:SetMaterialColor(Color(0, 0, 0))
	-- factionBack:SetMaterial(buttonMaterial)
	factionBack:SetText("return")
	factionBack:SetFont("qCaptureMenuButtonLabelFont")
	...
	-- history panel
	self.history = self:AddSubpanel("history")
	self.history:SetTitle("chooseHistory")

	self.historyPanel = self.history:Add("Panel")
	--self.historyPanel:SetWide(halfWidth + padding * 2)
	self.historyPanel:Dock(FILL)

	local historyBack = self.historyPanel:Add("ixMenuButton")
	-- historyBack:SetMaterial(buttonMaterial)
	-- historyBack:SetMaterialColor(Color(0, 0, 0))
	historyBack:SetText("return")
	historyBack:SetFont("qCaptureMenuButtonLabelFont")
	historyBack:SetContentAlignment(5)
	historyBack:SizeToContents()
	historyBack:Dock(BOTTOM)
	historyBack.DoClick = function()
		self.progress:DecrementProgress()
		self:SetActiveSubpanel("faction")
	end

	--[[local detalesProceed = self.detalesPanel:Add("qMenuButton")
	detalesProceed:SetText("proceed")
	detalesProceed:SetMaterialColor(Color(0, 0, 0))
	detalesProceed:SetMaterial(buttonMaterial)
	detalesProceed:SetContentAlignment(6)
	detalesProceed:SizeToContents()
	detalesProceed:Dock(BOTTOM)
	detalesProceed.DoClick = function()
		self.progress:IncrementProgress()
	...
	-- descriptionBack:SetMaterialColor(Color(0, 0, 0))
	-- descriptionBack:SetMaterial(buttonMaterial)
	descriptionBack:SetText("return")
	descriptionBack:SetFont("qCaptureMenuButtonLabelFont")
	...
	-- descriptionProceed:SetMaterialColor(Color(0, 0, 0))
	-- descriptionProceed:SetMaterial(buttonMaterial)
	descriptionProceed:SetText("proceed")
	descriptionProceed:SetFont("qCaptureMenuButtonLabelFont")
	...
	descriptionProceed.DoClick = function()
		if (self:VerifyProgression("description")) then
			...
			self.progress:IncrementProgress()
			self:SetActiveSubpanel("detales")
		end
	end

	-- detales panel
	self.detales = self:AddSubpanel("detales")
	self.detales:SetTitle("chooseDetales")

	local detalesLeft = self.detales:Add("Panel")
	detalesLeft:Dock(LEFT)
	detalesLeft:SetSize(halfWidth, halfHeight)

	self.detalesPanel = self.detales:Add("Panel")
	self.detalesPanel:SetWide(halfWidth + padding * 2)
	self.detalesPanel:Dock(RIGHT)

	local DLabel = self.detalesPanel:Add( "DLabel" )
	DLabel:Dock(TOP)
	DLabel:SetText( "В последствии их изменение невозможно!" )
	DLabel:SetFont("ixMainMenuFont")
	DLabel:SizeToContents()
	DLabel:SetTextColor(colorScheme)
	DLabel:SetContentAlignment(5)

	self.detalesModel = detalesLeft:Add("ixModelPanel")
	self.detalesModel:Dock(FILL)
	self.detalesModel:SetModel(self.factionModel:GetModel())
	self.detalesModel:SetFOV(modelFOV - 13)
	self.detalesModel.PaintModel = self.detalesModel.Paint

	local detalesBack = detalesLeft:Add("ixMenuButton")
	-- detalesBack:SetMaterial(buttonMaterial)
	-- detalesBack:SetMaterialColor(Color(0, 0, 0))
	detalesBack:SetText("return")
	detalesBack:SetFont("qCaptureMenuButtonLabelFont")
	detalesBack:SetContentAlignment(4)
	detalesBack:SizeToContents()
	detalesBack:Dock(BOTTOM)
	detalesBack.DoClick = function()
		self.progress:DecrementProgress()
		self:SetActiveSubpanel("description")
	end

	local detalesProceed = self.detalesPanel:Add("ixMenuButton")
	detalesProceed:SetText("proceed")
	-- detalesProceed:SetMaterialColor(Color(0, 0, 0))
	-- detalesProceed:SetMaterial(buttonMaterial)
	detalesProceed:SetContentAlignment(6)
	detalesProceed:SetFont("qCaptureMenuButtonLabelFont")
	detalesProceed:SizeToContents()
	detalesProceed:Dock(BOTTOM)
	detalesProceed.DoClick = function()
		if (self:VerifyProgression("detales")) then
			-- there are no panels on the attributes section other than the create button, so we can just create the character
			self.progress:IncrementProgress()
	...
	attributesBack:SetFont("qCaptureMenuButtonLabelFont")
	-- attributesBack:SetMaterial(buttonMaterial)
	-- attributesBack:SetMaterialColor(Color(0, 0, 0))
	...
	-- create:SetMaterialColor(Color(0, 0, 0))
	-- create:SetMaterial(buttonMaterial)
	create:SetContentAlignment(5)
	create:SetFont("qCaptureMenuButtonLabelFont")
	...
	self:AddPayloadHook("model", function(value)
		...
		if (faction) then
			...
			if (istable(model)) then
				...
				self.detalesModel:SetModel(model[1], model[2] or 0, model[3])
				...
				self.detalesModel:SetModel(model)
...
function PANEL:GetContainerPanel(name)
	...
	elseif (name == "detales") then
		return self.detalesPanel
	end
...
function PANEL:Populate()
	if (!self.bInitialPopulate) then
		...
		for _, v in SortedPairs(ix.faction.teams) do
			if (ix.faction.HasWhitelist(v.index)) then
				...
				button:SetFont("ixCaptureCharSelFont")
	...
	for k, v in SortedPairsByMemberValue(ix.char.vars, "index") do
		if (!v.bNoDisplay and k != "__SortedIndex") then
			...
			if (IsValid(panel)) then
				...
				label:SetFont("ixCaptureMenuButtonLabelFont")
				label:SetTextColor(colorScheme)
	...
	if (!self.bInitialPopulate) then
		...
		self.progress:AddSegment("История")

		self.progress:AddSegment("@description")

		self.progress:AddSegment("Детали")
