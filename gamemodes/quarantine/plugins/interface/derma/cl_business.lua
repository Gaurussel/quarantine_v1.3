...
local PANEL = {}
local panelMaterial = Material("materials/menu/canvas-3.png")
...
PANEL = {}
local buttonMaterial = Material("materials/menu/stroke-06.png")
...
function PANEL:Init()
	...
	self.dpanel = self:Add("DPanel")
	self.dpanel:SetSize(ScrW() * 0.6, ScrH() * 0.6)
	self.dpanel:SetPos(self:GetWide() * 0.25, self:GetTall() * 0.25)
	self.dpanel:MakePopup()
	self.dpanel.Paint = function(this, width, height)
		surface.SetDrawColor(20, 20, 20, 255)
		surface.DrawRect(0, 0, width, height)
	
		surface.SetDrawColor(45, 45, 45, 255)
		surface.SetMaterial(panelMaterial) 
	...
	self.categories = self.dpanel:Add("DScrollPanel")
	self.categories:Dock(LEFT)
	self.categories:SetWide(260)
	self.categories:SetTall(self.dpanel:GetTall())
	self.categories.Paint = function(this, w, h)
	end
	self.categories:DockPadding(5, 5, 5, 5)
	self.categories:DockMargin(10, 25, 0, 0)
	self.categoryPanels = {}

	self.scroll = self.dpanel:Add("DScrollPanel")
	self.scroll:Dock(FILL)

	self.search = self.dpanel:Add("DTextEntry")
	self.search:Dock(TOP)
	self.search:SetTall(36)
	self.search:SetFont("ixMediumFont")
	self.search:DockMargin(10 + self.dpanel:GetWide() * 0.35, 15, 15, 5)
	...
	self.checkout = self.dpanel:Add("qMenuButton")
	self.checkout:Dock(BOTTOM)
	self.checkout:SetMaterial(buttonMaterial)
	self.checkout:SetMaterialColor(color_black)
	self.checkout:SetTextColor(color_white)
	self.checkout:SetTall(36)
	self.checkout:SetFont("ixMediumFont")
	self.checkout:DockMargin(10, 0, 10, 15)
	...
	for category, realName in SortedPairs(self.categoryPanels) do
		...
		button:SetMaterialColor(Color(0, 0, 0, 255))
		button:Dock(TOP)
		button:SetTextColor(color_white)
		button:DockMargin(5, 5, 5, 0)
		button.DoClick = function(this)
			if (self.selected != this) then
				self.selected.selectedd = false
				self.selected = this
				this.selectedd = true
	...
	if (self.selected) then
		...
		self.selected.selectedd = true
	end

	self.close = self.categories:Add("qMenuButton")
	self.close:SetMaterial(buttonMaterial)
	self.close:SetMaterialColor(color_black)
...
vgui.Register("ixBusiness", PANEL, "DFrame")
