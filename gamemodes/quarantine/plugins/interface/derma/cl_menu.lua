...
function PANEL:Init()
	...
	-- setup
	self:SetPadding(ScreenScale(0), true)
	...
	local close = self.buttons:Add("qMenuButton2")
	...
	local characters = self.buttons:Add("qMenuButton2")
...
function PANEL:GetStandardSubpanelSize()
	return ScrW() - self:GetPadding() * 3, ScrH() - self:GetPadding() * 3 - self.topOffset
end
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	if (sectionParent) then
		...
		button = self.tabs:Add("qMenuSelectionButton")
		button:SetText(L(name))
		button:SizeToContents()
		button:Dock(LEFT)
		button:SetButtonList(self.tabs.buttons)

		for k, v in pairs(self.tabs:GetChildren()) do
			v:SetWide((self:GetWide() - self:GetPadding() * 2) / (#self.tabs.buttons + 1))
		end
