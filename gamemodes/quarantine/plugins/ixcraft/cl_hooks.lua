...
function PLUGIN:BuildCraftingMenu()
	if (table.IsEmpty(self.craft.GetCategories(LocalPlayer())) or LocalPlayer():Team() == FACTION_ADMIN) then
		return false
	end
end
...
function PLUGIN:PopulateRecipeTooltip(tooltip, recipe)
	...
	if (recipe.skills) then
		local tools = tooltip:AddRow("attribute")
		tools:SetText("АТРИБУТ:")
		...
		for k, v in pairs(recipe.skills) do
			level = v

			for k2, v2 in pairs(ix.attributes.list) do
				if k == k2 then
					skillString = skillString..v2.name..", "
				end
			end
		end
		...
		if (skillString != "") then
			...
			tools:SetText(string.sub(skillString, 0, #skillString-2)..", уровня "..level)
			tools:SizeToContents()
		end
	end

	if recipe.fire then
		local fire = tooltip:AddRow("fire")
		fire:SetText("Требуется огонь")
		fire:SetBackgroundColor(Color(250,125,5))
		fire:SizeToContents()
	end
