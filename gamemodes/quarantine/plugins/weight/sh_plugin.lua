...
function ix.weight.WeightString(weight, imperial)
	if (imperial) then
		...
		if (weight < 1) then -- The superior units of measurement.
			...
			return math.Round(weight, 2).." кг"
		end
	end
end

function ix.weight.BaseWeight(character)
	local maxWeight = ix.config.Get("maxWeight")
	local base = maxWeight + math.floor(maxWeight * character:GetAttribute("str", 0) / ix.config.Get("maxAttributes"))
	...
	function PLUGIN:PopulateItemTooltip(tooltip, item)
		...
		if (weight) then
			local row = tooltip:AddRowAfter("description", "weight")
				row:SetText("Вес: "..ix.weight.WeightString(weight, ix.option.Get("imperial", false)))
					...
					local barT = weight:Add("DLabel")
						...
						barT.Think = function()
							...
							if (ix.option.Get("imperial", false)) then
								...
								barT:SetText(math.Round(carry, 2).." кг / "..maxWeight.." кг")
							end
						end
