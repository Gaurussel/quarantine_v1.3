...
if (SERVER) then
	...
	ix.log.AddType("command", function(client, ...)
		...
		if (arg[2] and #arg[2] > 0) then
			Discord.Log(client:Name().." ("..client:SteamID()..") использовал команду "..arg[1].." "..arg[2], client)
			return L("%s used command '%s %s'.", client:Name(), arg[1], arg[2])
		else
			Discord.Log(client:Name().." ("..client:SteamID()..") использовал команду "..arg[1], client)
	...
	ix.log.AddType("connect", function(client, ...)
		Discord.Log(""..client:SteamName().." ("..client:SteamID()..") подключился к серверу.", client)
		return L("%s has connected.", client:SteamName())
	end, FLAG_NORMAL)

	ix.log.AddType("disconnect", function(client, ...)
		if (client:IsTimingOut()) then
			Discord.Log(""..client:Name().." ("..client:SteamID()..") отключился от сервера (вылет).", client)
			return L("%s (%s) has disconnected (timed out).", client:SteamName(), client:SteamID())
		else
			Discord.Log(""..client:Name().." ("..client:SteamID()..") отключился от сервера.", client)
	...
	ix.log.AddType("charCreate", function(client, ...)
		local arg = {...}
		Discord.Log(client:SteamName().." ("..client:SteamID()..") создал персонажа с именем "..arg[1], client)
		return L("%s created the character '%s'", client:SteamName(), arg[1])
	end, FLAG_SERVER)

	ix.log.AddType("charLoad", function(client, ...)
		local arg = {...}
		Discord.Log(client:SteamName().." ("..client:SteamID()..") загрузил персонажа с именем "..arg[1], client)
		return L("%s loaded the character '%s'", client:SteamName(), arg[1])
	end, FLAG_SERVER)

	ix.log.AddType("charDelete", function(client, ...)
		local arg = {...}
		Discord.Log(client:SteamName().." ("..client:SteamID()..") удалил персонажа "..arg[1], client)
	...
	ix.log.AddType("itemAction", function(client, ...)
		local arg = {...}
		local item = arg[2]
		Discord.Log(client:Name().." ("..client:SteamID()..") использовал ".."'"..arg[1].."' на "..item:GetName().."("..item:GetID()..")", client)
	...
	ix.log.AddType("itemDestroy", function(client, itemName, itemID)
		...
		if client:IsPlayer() then
			Discord.Log(name.." ("..client:SteamID()..") уничтожил "..itemName.." ("..itemID..")", client)
		end
	...
	ix.log.AddType("playerDeath", function(client, ...)
		local arg = {...}
		Discord.Log(client:Name().." ("..client:SteamID()..") был убит "..arg[1], client)
	...
	ix.log.AddType("inventoryAdd", function(client, characterName, itemName, itemID)
		Discord.Log(characterName.." ("..client:SteamID()..") получил ".."'"..itemName.."'".."("..itemID..")", client)
	...
	ix.log.AddType("inventoryRemove", function(client, characterName, itemName, itemID)
		Discord.Log(characterName.." ("..client:SteamID()..") потерял ".."'"..itemName.."'".." ("..itemID..")", client)
	...
	ix.log.AddType("roll", function(client, value, max)
		Discord.Log(client:Name().." ("..client:SteamID()..") выполнил ролл и получил число "..value.." из "..max, client)
