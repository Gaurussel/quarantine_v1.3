...
local function WritePreciseVector(vector)
	...
	if trace.Hit or trace.HitSky then
		...
		if not bullet.noRicochet then
			if not trace.HitSky then
				...
				local _, _, penStr, penRange = wepData:getEffectiveRange()
...
if CLIENT then
	net.Receive("CW20_PHYSBUL", function(len, pl)
		...
		struct.position = ReadPreciseVector()
		struct.direction = ReadPreciseVector()
		...
		local index = table.insert(CustomizableWeaponry.physicalBulletBuffer[struct.player], struct)
		...
		for ply, data in pairs(self.physicalBulletBuffer) do
			if not IsValid(ply) then -- player does not exist, don't draw his bullets
				...
				for k, v in ipairs(data) do
					...
					local norm = v.direction:GetNormal()

					if v.player ~= localPlayer and norm:DotProduct(eyeAngles:GetNormalized()) < 0 and CT > bulletWhizTime and v.position:Distance(playerPos) <= self.WHIZ_DISTANCE then -- emit a whiz sound
					...
					if v.isTracer or v.player == localPlayer then
