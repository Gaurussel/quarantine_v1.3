...
function CustomizableWeaponry:buildAttachmentString()
	...
	for _, v in ipairs(CustomizableWeaponry.registeredAttachments) do
...
local cw_keep_attachments_post_death = GetConVar("cw_keep_attachments_post_death")
...
function CustomizableWeaponry:postSpawn()
	...
	if CustomizableWeaponry.useAttachmentPossessionSystem and not CustomizableWeaponry.suppressOnSpawnAttachments then
		if SERVER then
			local keepAttachments = cw_keep_attachments_post_death:GetInt() >= 1
	...
	for k, v in pairs(player.GetAll()) do
		if v:IsAdmin() then
			for k2, v2 in ipairs(CustomizableWeaponry.registeredAttachments) do
				v:ConCommand(v2.clcvar .. space .. GetConVar(v2.cvar):GetInt())
			end
		end
	end
end
